...
# Usage: create_batch_categories.sh <session> [restapi_url]
...
TEMPLATE_JSON="${SCRIPT_DIR}/create_batch_categories.template.json"
...
REQ_FILE="${TMPDIR:-/tmp}/create_batch_categories.$$.json"
...
echo "Posting POPULATE-BATCH-CATEGORY to ${RESTAPI_URL}" >&2
RESP_FILE="${TMPDIR:-/tmp}/create_batch_categories.response.$$.json"
...
json_status=$(jq -r '.response_status // .status // empty' "$RESP_FILE" 2>/dev/null || true)
