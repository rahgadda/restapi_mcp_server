...
# Usage: create_party.sh <session> [restapi_url]
...
TEMPLATE_JSON="${SCRIPT_DIR}/create_party.template.json"
...
REQ_FILE="${TMPDIR:-/tmp}/create_party.$$.json"
...
echo "Posting CREATE-PARTY to ${RESTAPI_URL}" >&2
RESP_FILE="${TMPDIR:-/tmp}/create_party.response.$$.json"
